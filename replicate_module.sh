...
for N in `seq ${COUNT}`; do
  ...
  sed -i.bak -e "s/gax-test00/gax-test${suffix}/" "${module}/src/test/java/com/google/api/gax/httpjson/HttpJsonDirectServerStreamingCallableTest.java"
  sed -i.bak -e "s/gax_test00/gax_test${suffix}/" "${module}/src/test/java/com/google/api/gax/httpjson/HttpJsonDirectServerStreamingCallableTest.java"
